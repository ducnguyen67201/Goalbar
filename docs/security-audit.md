...
`cargo audit` sees `rsa 0.9.10` in `Cargo.lock` through SQLx's optional macro/database metadata. The application disables SQLx default features and enables only the Tokio, SQLite, Chrono, and UUID features. `cargo tree --target all -i rsa` reports no dependency path, so the affected RSA implementation is not compiled or reachable in Goalbar.
