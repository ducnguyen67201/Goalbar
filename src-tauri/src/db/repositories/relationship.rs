...
impl RelationshipRepository {
    ...
    pub async fn conversations(&self) -> AppResult<Vec<ConversationSummary>> {
        let rows = sqlx::query("SELECT c.id, c.platform, c.remote_id, c.kind, COALESCE(r.display_name, c.notification_display_name, 'Unknown person') AS display_name, COALESCE((SELECT body FROM messages m WHERE m.conversation_id = c.id ORDER BY sent_at DESC LIMIT 1), '') AS preview, c.unread_count, c.reply_capability, c.remote_url, bi.profile_url, CASE WHEN bi.conversation_id IS NOT NULL THEN 'browser_scan' ELSE c.source END AS source, c.content_state, c.updated_at FROM conversations c LEFT JOIN relationships r ON r.id = c.relationship_id LEFT JOIN browser_inbox_ingestions bi ON bi.conversation_id = c.id ORDER BY c.unread_count DESC, c.updated_at DESC")
            .fetch_all(&self.pool)
            .await?;
        rows.iter()
            .map(|row| {
